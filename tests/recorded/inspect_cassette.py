...
from tests.recorded.cassette import _decode_body_json, _decode_body_text, _stream_payloads_from_body
...
def _request_payload(interaction: dict[str, Any]) -> dict[str, Any]:
    ...
    payload = _decode_body_json(request.get("body"))
...
def cassette_summary(path: Path) -> list[dict[str, Any]]:
    ...
    for index, interaction in enumerate(data.get("interactions", [])):
        ...
        stream_payloads = _stream_payloads_from_body(response.get("body"))
        rows.append(
            {
                "index": index,
